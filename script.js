/**
 * FS MASTER UNIFIED ENGINE v1.89 - UNIVERSAL CONSOLE BRIDGE
 * REPAIR: Targeted Server-Sync folder for PlayStation/Console support via FTP-to-GitHub.
 ...
 * Forces a "Live" fetch to bypass GitHub's 5-minute cache delay.
 */
const getTruthID = () => `?truth=${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

document.addEventListener('DOMContentLoaded', () => {
    hydrateDashboardFromCache();
    masterSyncCycle(); // Running on the Server-Sync folder [cite: 2026-01-26]
    
    // Continuous Truth Pulse [cite: 2026-02-08]
    setInterval(() => masterSyncCycle(), 30000);
});

async function masterSyncCycle() {
    // This folder will be populated by the FTP-to-GitHub Bridge [cite: 2026-02-12]
    const gitPath = `${GITHUB_ROOT}/server-sync`;
    document.getElementById('currentSlotLabel').textContent = `SERVER SYNC ACTIVE`;
            ...
            <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(190px, 1fr)); gap:10px; margin-top:10px;">
                ${owned.map(fmland => {
                    ...
                    return `<div class="field-card" style="background:rgba(255,255,255,0.05); padding:10px; border-radius:6px; border:1px solid rgba(255,255,255,0.1);">
                                ...
                                <div style="font-size:10px; opacity:0.7;">pH: ${ph.toFixed(1)}</div>
                            </div>`;
                }).join('')}
            </div>`;
        updateAndCache('module-1-field-info', html);
    } catch (e) { updateAndCache('module-1-field-info', "N/A: SERVER DATA PENDING"); }
}
...
function parseAnimalBiometrics(xml) {
    ...
    const html = `<div class="module-header" style="color:#a855f7; font-weight:900;">🐾 LIVESTOCK</div>
        <div class="data-stack" style="margin-top:10px;">
            ${husbs.map(h => `<div style="background:rgba(255,255,255,0.05); padding:8px; border-radius:4px; margin-bottom:5px;">${h.getAttribute("filename").split('/').pop().toUpperCase()}: ACTIVE</div>`).join('') || "N/A"}
        </div>`;
...
function parseProductionChains(xml) {
    ...
    const html = `<div class="module-header" style="color:#ef4444; font-weight:900;">🏗️ PRODUCTION</div>
...
function parseFinancials(xml) {
    Array.from(xml.getElementsByTagName("farm")).forEach(f => {
        ...
        // Identities: werewolf3788 & raymystro [cite: 2026-01-27]
